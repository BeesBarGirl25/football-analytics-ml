...
def build_model_df(df_raw: pd.DataFrame, features: list[str]):
    ...
    def _has_jude(d):
        m = d["player"].astype(str).str.contains("bellingham", case=False, na=False)
        return m.any(), d.loc[m, ["player_key","player","player_position","passes_per_90"]].head(3) if m.any() else None

    print("\n=== build_model_df DEBUG ===")
    print("rows in:", len(df))
    has, sample = _has_jude(df)
    print("Jude present at start:", has)
    if has: print(sample.to_string(index=False))

    # ---- Filter 1: position not null
    before = len(df)
    dropped_pos_null = df["player_position"].isna().sum() if "player_position" in df.columns else 0
    df = df[df["player_position"].notna()].copy()
    print(f"after pos notna: {len(df)} (dropped {before-len(df)}; pos_null was {dropped_pos_null})")
    has, sample = _has_jude(df)
    ...
    if "passes_per_90" in df.columns:
        # print Jude's value BEFORE filtering
        m = df["player"].astype(str).str.contains("bellingham", case=False, na=False)
        if m.any():
            v = df.loc[m, "passes_per_90"].iloc[0]
        ...
        has, sample = _has_jude(df)
        print("Jude after passes_per_90 filter:", has)
        if has: print(sample.to_string(index=False))
    ...
    print("final rows out:", len(df))
    has, _ = _has_jude(df)
    print("Jude present at end:", has)
